...
int am_disable_irq(void)
{
	...
	mipi_adap_reg_wr(MIPI_ADAPT_IRQ_MASK0, ALIGN_IO, 0);
...
void am_adap_frontend_start(void)
{
	...
	adap_wr_reg_bits(CSI2_GEN_CTRL0, FRONTEND_IO, 0xf, 0, 4);

	if (frontend1_flag) {
		adap_wr_reg_bits(CSI2_GEN_CTRL0 + FTE1_OFFSET, FRONTEND_IO, 0xf, 0, 4);
	}
...
int am_adap_frontend_init(void)
{
	...
	if (para.mode == DIR_MODE) {
		if (para.path == PATH0)
			mipi_adap_reg_wr(CSI2_GEN_CTRL0, FRONTEND_IO, 0x00010000);//bit[0] 1:enable virtual channel 0
	} else if (para.mode == DDR_MODE) {
		if (para.path == PATH0)
			mipi_adap_reg_wr(CSI2_GEN_CTRL0, FRONTEND_IO, 0x00010010);
	} else if (para.mode == DOL_MODE) {
		mipi_adap_reg_wr(CSI2_GEN_CTRL0, FRONTEND_IO, 0x000110a0);

		if (frontend1_flag) {
				mipi_adap_reg_wr(CSI2_GEN_CTRL0 + FTE1_OFFSET, FRONTEND_IO, 0x000110a0);
		}
	} else {
	...
	adap_wr_reg_bits(CSI2_X_START_END_ISP , FRONTEND_IO, para.offset.offset_x, 0, 16);
	...
	if (para.mode == DOL_MODE) {
		if (para.type == DOL_VC) {                          //for ov
			...
			if (frontend1_flag)
				mipi_adap_reg_wr(CSI2_VC_MODE + FTE1_OFFSET, FRONTEND_IO, 0x22110000);
			...
			} else if (para.fmt == AM_RAW12) {    //to be verified
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_L, FRONTEND_IO, 0xffccdbdb);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_H, FRONTEND_IO, 0xffffffff);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_TO_VC_L, FRONTEND_IO, 0x112424);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_TO_VC_H, FRONTEND_IO, 0x0);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_TO_IGNORE_L, FRONTEND_IO, 0x2020);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_TO_IGNORE_H, FRONTEND_IO, 0x0);
			} else {
			...
			if (para.fmt == AM_RAW10 ) {
				...
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_A_L, FRONTEND_IO, 0x90909090);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_A_H, FRONTEND_IO, 0x55);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_B_L, FRONTEND_IO, 0x6e6e6e6e);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_B_H, FRONTEND_IO, 0xffffff00);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_B_L, FRONTEND_IO, 0x90909090);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_B_H, FRONTEND_IO, 0xaa);
			} else if (para.fmt == AM_RAW12) {
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_A_L, FRONTEND_IO, 0xff000101);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_A_H, FRONTEND_IO, 0xffffffff);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_A_L, FRONTEND_IO, 0x112424);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_A_H, FRONTEND_IO, 0x0);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_B_L, FRONTEND_IO, 0xff000101);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_B_H, FRONTEND_IO, 0xffffffff);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_B_L, FRONTEND_IO, 0x222424);
				mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_B_H, FRONTEND_IO, 0x0);
			} else {
				pr_err("raw format to be supported");
			}
#endif
			...
			if (frontend1_flag) {
				...
				if (para.fmt == AM_RAW10 ) {
					...
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_L + FTE1_OFFSET, FRONTEND_IO, 0xffccdbdb);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_H + FTE1_OFFSET, FRONTEND_IO, 0xffffffff);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_TO_VC_L + FTE1_OFFSET, FRONTEND_IO, 0x112424);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_TO_VC_H + FTE1_OFFSET, FRONTEND_IO, 0x0);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_TO_IGNORE_L + FTE1_OFFSET, FRONTEND_IO, 0x2020);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_TO_IGNORE_H + FTE1_OFFSET, FRONTEND_IO, 0x0);
				} else {
					pr_err("raw format to be supported");
				}
				...
				if (para.fmt == AM_RAW10 ) {
					...
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_A_L + FTE1_OFFSET, FRONTEND_IO, 0x90909090);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_A_H + FTE1_OFFSET, FRONTEND_IO, 0x55);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_B_L + FTE1_OFFSET, FRONTEND_IO, 0x6e6e6e6e);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_B_H + FTE1_OFFSET, FRONTEND_IO, 0xffffff00);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_B_L + FTE1_OFFSET, FRONTEND_IO, 0x90909090);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_B_H + FTE1_OFFSET, FRONTEND_IO, 0xaa);
				} else if (para.fmt == AM_RAW12) {
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_A_L + FTE1_OFFSET, FRONTEND_IO, 0xff000101);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_A_H + FTE1_OFFSET, FRONTEND_IO, 0xffffffff);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_A_L + FTE1_OFFSET, FRONTEND_IO, 0x112424);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_A_H + FTE1_OFFSET, FRONTEND_IO, 0x0);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_B_L + FTE1_OFFSET, FRONTEND_IO, 0xff000101);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_MASK_B_H + FTE1_OFFSET, FRONTEND_IO, 0xffffffff);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_B_L + FTE1_OFFSET, FRONTEND_IO, 0x222424);
					mipi_adap_reg_wr(CSI2_VC_MODE2_MATCH_B_H + FTE1_OFFSET, FRONTEND_IO, 0x0);
				} else {
					pr_err("raw format to be supported");
				}

#endif
				adap_wr_reg_bits(CSI2_X_START_END_MEM + FTE1_OFFSET, FRONTEND_IO, 0xc, 0, 16);
				...
				adap_wr_reg_bits(CSI2_Y_START_END_MEM + FTE1_OFFSET, FRONTEND_IO,
							short_exp_offset + para.img.height - 1, 16, 16);
				//set short exposure offset
				adap_wr_reg_bits(CSI2_X_START_END_ISP + FTE1_OFFSET, FRONTEND_IO, 0xc, 0, 16);
...
int am_adap_alig_init(void)
{
	...
	if (para.mode == DIR_MODE) {
		...
		mipi_adap_reg_wr(MIPI_ADAPT_ALIG_CNTL8, ALIGN_IO, 0x05231020);
	} else if (para.mode == DDR_MODE) {
		mipi_adap_reg_wr(MIPI_ADAPT_ALIG_CNTL6, ALIGN_IO, 0x00fff001);
		mipi_adap_reg_wr(MIPI_ADAPT_ALIG_CNTL7, ALIGN_IO, 0x0);
		mipi_adap_reg_wr(MIPI_ADAPT_ALIG_CNTL8, ALIGN_IO, 0x00000020);
	} else if (para.mode == DOL_MODE) {
...
int am_adap_init(void)
{
	...
	if (para.mode == DOL_MODE && g_adap->f_end_irq == 0) {
