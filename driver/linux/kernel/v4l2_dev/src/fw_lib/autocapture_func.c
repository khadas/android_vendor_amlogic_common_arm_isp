...
#define FW_DS2_AUTO_CAP_BASE       0x71200000
#define FW_DS2_AUTO_CAP_SIZE       0x546000
...
static int autocapture_fops_mmap( struct file *f, struct vm_area_struct *vma )
{
	...
	if(p_ctx->get_fr_ds == GET_FR)
		...
		fw_addr_read = autowrite_ds2_start_address_read();
#endif
...
static ssize_t autocapture_fops_read( struct file *file, char __user *buf, size_t count, loff_t *ppos )
{
	...
	else if(p_ctx->get_fr_ds == GET_DS2)
	{
		...
		p_ctx->autocap_frame[dma_ds2].count = autowrite_ds2_writer_frame_wcount_read();
...
uint32_t autocap_get_frame_info(struct autocapture_context *p_ctx, uint32_t type)
{
	...
	uint32_t fps = 30;
#endif
	do_gettimeofday(&p_ctx->t_frm);

	p_ctx->autocap_frame[type].endtime = (p_ctx->t_frm.tv_sec*1000000 + p_ctx->t_frm.tv_usec)/1000;
	...
	switch(type)
	{
		...
		case dma_ds2:
			...
			p_ctx->autocap_frame[dma_ds2].memory_size = autowrite_ds2_writer_memsize_read();
			p_ctx->autocap_frame[dma_ds2].count = autowrite_ds2_writer_frame_wcount_read();
			p_ctx->autocap_frame[dma_ds2].s_address = autowrite_ds2_start_address_read();
			if(autowrite_ds2_drop_mode_status())
			{
				if(autowrite_ds2_drop_write_read())
					fps = 30 * autowrite_ds2_drop_write_read() / (autowrite_ds2_drop_write_read() + autowrite_ds2_drop_jump_read());
			}
		    break;
		default:
			realcount = 0;
...
uint32_t autocap_get_frame_addr(struct autocapture_context *p_ctx, struct frame_info *frm)
{
	...
	if((index > p_ctx->autocap_frame[type].realcount) || (type > dma_ds2))
	{
		p_ctx->autocap_frame[type].n_address = 0;
...
long autocapture_fops_ioctl (struct file *pfile, unsigned int cmd, unsigned long args)
{
	...
	switch (cmd)
	{
		...
		case IOCTL_GET_DS1_INFO:
			...
			break;
		case IOCTL_GET_DS2_INFO:
			rc = copy_to_user((void *)args, &p_ctx->autocap_frame[dma_ds2], sizeof(struct autocap_image));
			if ( rc ) {
				 LOG( LOG_CRIT, "copy_to_user failed, rc: %d.", rc );
			}
			break;
		...
		case IOCTL_STOP_AUTO_DS1:
			...
			break;
		case IOCTL_STOP_AUTO_DS2:
			autowrite_ds2_stop();
			autocap_get_frame_info(p_ctx, dma_ds2);
		    p_ctx->get_fr_ds = GET_DS2;
			break;
...
void autocapture_initialize( autocapture_fsm_t *p_fsm )
{
	...
	m_cfg_ds2.p_type = dma_ds2;
	m_cfg_ds2.planes = 1;
	m_cfg_ds2.frame_size0 = (1280 * 720 * 3);
	fw_auto_cap_init(p_ctx, (void *)&m_cfg_ds2);
#endif
...
static int autocap_v4l2_stream_copy_thread( void *data )
{
	...
	while ( !kthread_should_stop() ) {
		...
		if(p_ctx->pstream[dma_ds1] && p_ctx->pstream[dma_ds1]->stream_started)
		{
		 ...
		 if(p_ctx->pstream[dma_ds2] && p_ctx->pstream[dma_ds2]->stream_started)
		 {
			 res = autocap_pullbuf(p_ctx->fw_id, dma_ds2, f_buff);
			 if(res == 0)
				continue;
		}
...
uint32_t autocap_pullbuf(int ctx_id, int d_type, tframe_t f_buff)
{
	...
	if(kfifo_len(&p_ctx->read_fifo[d_type]) > 0)
	{
		...
		else if(d_type == dma_ds2)
		{
		    system_memcpy(phys_to_virt(p_ctx->d_buff[d_type][p_buff].primary.address), phys_to_virt(FW_DS2_AUTO_CAP_BASE), \
			    p_ctx->d_buff[d_type][p_buff].primary.size+p_ctx->d_buff[d_type][p_buff].secondary.size);

		    p_ctx->p_fsm->p_fsm_mgr->p_ctx->settings.callback_ds2(ctx_id, &p_ctx->d_buff[d_type][p_buff], NULL);
		}
	}
...
void fw_auto_cap_init(struct autocapture_context *p_ctx, void *cfg_info)
{
	...
	case dma_ds2:
		if ( m_cfg->planes ==1) {
			a_cfg->frame_buffer_start0 = FW_DS2_AUTO_CAP_BASE;
			a_cfg->frame_buffer_size0 = FW_DS2_AUTO_CAP_SIZE;
			a_cfg->frame_buffer_start1 = 0x00000000;
			a_cfg->frame_buffer_size1 = 0x0;
		} else if (m_cfg->planes == 2) {
			a_cfg->frame_buffer_start0 = FW_DS1_AUTO_CAP_BASE;
	...
	} else if (a_cfg->p_type == dma_ds2) {
		auto_write_ds2_init(a_cfg);
	} else {
