...
#include "IMX334_seq.h"
#include "IMX334_config.h"
...
static imx334_private_t imx334_ctx;
...
static sensor_mode_t supported_modes[] = {
    {
        ...
        .bayer = BAYER_RGGB,
        .dol_type = DOL_NON,
        .num = SENSOR_IMX334_SEQUENCE_1080P_30FPS_10BIT_4LANE,
    },
    {
        .wdr_mode = WDR_MODE_LINEAR, // 4 Lanes
        .fps = 30 * 256,
        ...
        .bayer = BAYER_RGGB,
        .dol_type = DOL_NON,
        .num = SENSOR_IMX334_SEQUENCE_4K_30FPS_10BIT_4LANE,
    },
    {
        ...
        .bayer = BAYER_RGGB,
        .dol_type = DOL_VC,
        .num = SENSOR_IMX334_SEQUENCE_1080P_30FPS_10BIT_4LANE_WDR,
    },
    {
        .wdr_mode = WDR_MODE_FS_LIN,
        .fps = 30 * 256,
        ...
        .bayer = BAYER_RGGB,
        .dol_type = DOL_VC,
        .num = SENSOR_IMX334_SEQUENCE_4K_30FPS_10BIT_4LANE_WDR,
    },
};

//-------------------------------------------------------------------------------------
#if SENSOR_BINARY_SEQUENCE
static const char p_sensor_data[] = SENSOR_IMX334_SEQUENCE_DEFAULT;
static const char p_isp_data[] = SENSOR_IMX334_ISP_CONTEXT_SEQ;
#else
static const acam_reg_t **p_sensor_data = imx334_seq_table;
...
{
    sensor_context_t *p_ctx = ctx;
    imx334_private_t *p_imx334 = (imx334_private_t *)p_ctx->sdrv;
    ...
    case WDR_MODE_FS_LIN:
        ...
        if ( p_ctx->int_time_S != *int_time_S || p_ctx->int_time_L != *int_time_L ) {
            ...
            p_imx334->shr0 = p_ctx->frame - *int_time_L - 1;
            if ( p_imx334->shr0 < 20 ) p_imx334->shr0 = 20;
            p_imx334->shr1 = p_imx334->rhs1 - *int_time_S - 1;
            if ( p_imx334->shr1 < 9 ) p_imx334->shr1 = 9;
        }
        break;
    }
}
...
static uint32_t sensor_vmax_fps( void *ctx, uint32_t framerate )
{
    ...
    if ( p_ctx->wdr_mode == WDR_MODE_LINEAR ) {
        ...
        param->integration_time_limit = vmax - 6;
        param->integration_time_max = vmax - 6;
    } else {
        p_ctx->max_L = 2 * vmax - 4;
...
static void sensor_update( void *ctx )
{
    ...
    if ( p_ctx->int_cnt || p_ctx->gain_cnt || p_ctx->dgain_change ) {
        ...
        if ( p_ctx->gain_cnt || p_ctx->dgain_change ) {
            ...
            acamera_sbus_write_u8( p_sbus, 0x30eb, (reg_gain >> 8 ) & 0x07 );
            acamera_sbus_write_u8( p_sbus, 0x30ea, (reg_gain >> 0 ) & 0xFF );
        }
        ...
        if ( p_ctx->int_cnt ) {
            ...
            case WDR_MODE_LINEAR:
                acamera_sbus_write_u8( p_sbus, 0x3059, ( p_imx334->shr0 >> 8 ) & 0xFF );
                acamera_sbus_write_u8( p_sbus, 0x3058, ( p_imx334->shr0 >> 0 ) & 0xFF );
                break;
            case WDR_MODE_FS_LIN:
                p_imx334->shr1_old = p_imx334->shr1;
                p_imx334->shr0_old = p_imx334->shr0;

                p_imx334->shr0_old = (p_imx334->shr0_old >> 2) << 2;
                if ( p_imx334->shr0 < 20 ) p_imx334->shr0 = 20;

                p_imx334->shr1_old = ((p_imx334->shr1_old >> 2) << 2) + 1;
                if ( p_imx334->shr1 < 9 ) p_imx334->shr1 = 9;

                // SHR0
                acamera_sbus_write_u8( p_sbus, 0x3059, ( p_imx334->shr0_old >> 8 ) & 0xFF );
                acamera_sbus_write_u8( p_sbus, 0x3058, ( p_imx334->shr0_old >> 0 ) & 0xFF );

                // SHR1
                acamera_sbus_write_u8( p_sbus, 0x305d, ( p_imx334->shr1_old >> 8 ) & 0xFF );
                acamera_sbus_write_u8( p_sbus, 0x305c, ( p_imx334->shr1_old >> 0 ) & 0xFF );
                break;
            }
        }
...
static uint16_t sensor_get_id( void *ctx )
{
    ...
    sensor_id |= acamera_sbus_read_u8(&p_ctx->sbus, 0x3309) << 8;
    sensor_id |= acamera_sbus_read_u8(&p_ctx->sbus, 0x3308);

    if ((sensor_id != SENSOR_CHIP_ID) && (sensor_id != 0x43C)) {
        LOG(LOG_CRIT, "%s: Failed to read sensor imx334 id, %d\n", __func__, sensor_id);
        return 0xFFFF;
    }

    LOG(LOG_INFO, "%s: success to read sensor imx334: %x\n", __func__, sensor_id);
...
static void sensor_set_mode( void *ctx, uint8_t mode )
{
    ...
    uint8_t r = acamera_sbus_read_u8( p_sbus, 0x3018 );
    LOG(LOG_CRIT,"sensor resolution mode, r = %d", r);
    switch ( r ) {
    case 0: // HD 4K
        // RHS1 should be 4n+1 and (SHR1+2) <= RHS1 <= (SHR0 - 9)
        // SHS1 < (BRL * 2)
        // FSC = 2 * Vmax = 5400
        p_imx334->rhs1 = 273;

        // Long integration time = FSC - SHR0
        // SHR0 should be 2n and (SHS1 + 9) <= SHR0 <= (FSC-2)
        p_ctx->max_L = 2 * p_ctx->vmax - 2;

        // Short integration time = RHS1 - SHR1
        // Shutter time SHR1 should be 2n+1 and 9 <= SHR1 <= (RHS1 - 2)
        p_ctx->max_S = p_imx334->rhs1 - 2;
        break;
    case 4:  //HD 1080P
        p_imx334->rhs1 = 157;
        p_ctx->max_L = 2 * p_ctx->vmax - 2;
    ...
    if ( param->modes_table[mode].wdr_mode == WDR_MODE_LINEAR ) {
        param->integration_time_limit = p_ctx->vmax - 6;
        param->integration_time_max = p_ctx->vmax - 6;
    } else {
        param->integration_time_limit = p_ctx->max_S;
        param->integration_time_max = p_ctx->max_S;
...
static void sensor_test_pattern( void *ctx, uint8_t mode )
{
    ...
    sensor_load_sequence( p_sbus, p_ctx->seq_width, p_sensor_data, SENSOR_IMX334_SEQUENCE_DEFAULT_TEST_PATTERN );
}
...
void sensor_deinit_imx334( void *ctx )
{
    sensor_context_t *t_ctx = ctx;
...
static sensor_context_t *sensor_global_parameter(void* sbp)
{
    ...
    sensor_ctx.pixel_clock = 81526500;//74250000;
    sensor_ctx.s_fps = 30;
...
//--------------------Initialization------------------------------------------------------------
void sensor_init_imx334( void **ctx, sensor_control_t *ctrl, void* sbp)
{
...
int sensor_detect_imx334( void* sbp)
{
    int ret = 0;
    sensor_ctx.sbp = sbp;
    sensor_bringup_t* sensor_bp = (sensor_bringup_t*) sbp;
