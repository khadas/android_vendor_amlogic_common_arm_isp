...
static void dw9714_vcm_move( void *ctx, uint16_t position )
{
    ...
    step = ( position ) / (p_ctx->param.min_step);
    LOG( LOG_DEBUG, "%s min_step = %d, position = 0x%x, step = 0x%x",
            __func__, p_ctx->param.min_step, position, step );

    step = step & 0x3FF ; // 10 bits data.
    ...
    data[1] = ( (step & 0x0f) << 4 ) + data[1] & 0x0f; // KEEP s[3:0]
    ...
    *ctx = &g_dw9714_ctx;
    ...
    if (0 == g_dw9714_ctx.inited)
    {
        ...
        if (ret != 0) {
            ...
            pr_info("%s:success to add dw9714 i2c driver\n", __func__);
            g_dw9714_ctx.inited = 1;
        }
    } else {
        pr_info("%s: dw9714 i2c driver has been registered\n", __func__);
    }
}
