...
static const struct aml_format adap_cap_support_format[] = {
	...
	{0, 0, 0, V4L2_PIX_FMT_SBGGR8, 1, 8},
	{0, 0, 0, V4L2_PIX_FMT_SRGGB8, 1, 8},
	{0, 0, 0, V4L2_PIX_FMT_SGBRG8, 1, 8},
	{0, 0, 0, V4L2_PIX_FMT_SGRBG8, 1, 8},
	{0, 0, 0, V4L2_PIX_FMT_SBGGR10, 1, 10},
	{0, 0, 0, V4L2_PIX_FMT_SRGGB10, 1, 10},
	{0, 0, 0, V4L2_PIX_FMT_SGBRG10, 1, 10},
	{0, 0, 0, V4L2_PIX_FMT_SGRBG10, 1, 10},
	{0, 0, 0, V4L2_PIX_FMT_SBGGR12, 1, 12},
	{0, 0, 0, V4L2_PIX_FMT_SRGGB12, 1, 12},
	{0, 0, 0, V4L2_PIX_FMT_SGBRG12, 1, 12},
	{0, 0, 0, V4L2_PIX_FMT_SGRBG12, 1, 12},
};
		...
		if (b_current_filled) {
			...
			if ( (vd->frm_cnt % 100) == 0 ) {
				...
				if (endtime_ms > begtime_ms) {
					printk("adap fps - 100 frames %ld ms\n", (endtime_ms - begtime_ms) );
				}
...
int aml_adap_video_register(struct adapter_dev_t *adap_dev)
{
	...
	for (id = 0; id < AML_ADAP_STREAM_MAX; id++) {
		...
		video->id = id;
		video->belong_cam_idx = adap_dev->index;
		snprintf(video->name, sizeof(video->name), AML_VIDEO_NAME, video->belong_cam_idx, video->id);
