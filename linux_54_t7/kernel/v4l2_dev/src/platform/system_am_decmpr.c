...
int aml_cmpr_init(int tnr_cmpr_en, int w, int h){
	if(tnr_cmpr_en) {
		...
		cmpr_tnr_line_stride = 3840*3;//mipi_isp_line_stride(w);    //should match tnr stide
		cmpr_tnr_lossless = 1;//TNR_CMPR_LOSSLESS ? 1 : 0;
		...
		s_cmprcore_tnr_sub0.reg_xphase_ofst = 0;
		s_cmprcore_tnr_sub0.reg_yphase_ofst = 0;
		s_cmprcore_tnr_sub0.reg_wdr_mode_en = 0;
		s_cmprcore_tnr_sub0.reg_wdr_delay_line = 1;
		...
		if(cmpr_tnr_temp3_en==1) {
			...
			s_cmprcore_tnr_sub1.reg_xphase_ofst = 0;
			s_cmprcore_tnr_sub1.reg_yphase_ofst = 0;
			s_cmprcore_tnr_sub1.reg_wdr_mode_en = 0;
			s_cmprcore_tnr_sub1.reg_wdr_delay_line = 1;
	   ...
	   //
	   //cmpr_wr(TWSUB0_ISP_LOSS_PIC_SIZE, 0x0);
	   //cmpr_wr(TRSUB0_ISP_LOSS_PIC_SIZE, 0x0);
	...
	pr_err("init decmpr--------------------- success w%d h%d\n",w,h);
	return 0;
}
