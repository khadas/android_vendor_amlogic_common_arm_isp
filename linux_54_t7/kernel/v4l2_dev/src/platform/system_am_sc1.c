...
extern u32 sc2_isr_count;
u32 sc1_isr_count;
static struct am_sc1 *g_sc;
...
static ssize_t sc1_frame_read(struct device *dev,
...
static ssize_t sc1_frame_write(struct device *dev,
...
static DEVICE_ATTR(sc1_frame, S_IRUGO | S_IWUSR, sc1_frame_read, sc1_frame_write);
    ...
    if (start_delay_cnt < start_delay_th) {
        ...
        if (start_delay_cnt == start_delay_th) {
            ...
            if (g_sc->crop_refresh_flag) {
                am_sc1_hw_init(0, g_sc->crop_refresh_flag);
...
int am_sc1_parse_dt(struct device_node *node, int port)
{
    ...
    if (t_sc == NULL) {
        pr_err("%s: Failed to alloc isp-sc1\n", __func__);
        return -1;
    }
    ...
    device_create_file(&(t_sc->p_dev->dev), &dev_attr_sc1_frame);
    t_sc->port = port;
    g_sc = t_sc;
...
#endif
    if (g_sc->p_dev != NULL)
        device_remove_file(&(g_sc->p_dev->dev), &dev_attr_sc1_frame);
...
void am_sc1_api_dma_buffer(tframe_t * data, unsigned int index)
{
    unsigned long flags;
...
uint32_t am_sc1_get_width(void)
{
    if (!g_sc) {
...
void am_sc1_set_width(uint32_t src_w, uint32_t out_w)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
uint32_t am_sc1_get_startx(void)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return 0;
...
uint32_t am_sc1_get_starty(void)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return 0;
...
void am_sc1_set_fps(uint32_t c_fps, uint32_t t_fps)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
    ...
    pr_err("c_fps:%d, t_fps:%d\n", c_fps, t_fps);
}

uint32_t am_sc1_get_fps(void)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return 0;
...
void am_sc1_set_startx(uint32_t startx)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
void am_sc1_set_starty(uint32_t starty)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
uint32_t am_sc1_get_crop_width(void)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return 0;
...
uint32_t am_sc1_get_crop_height(void)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return 0;
...
void am_sc1_set_crop_width(uint32_t c_width)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
void am_sc1_set_crop_height(uint32_t c_height)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
void am_sc1_set_src_width(uint32_t src_w)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
void am_sc1_set_src_height(uint32_t src_h)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
uint32_t am_sc1_get_height(void)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return -1;
...
uint32_t am_sc1_get_output_format(void)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return -1;
...
void am_sc1_set_height(uint32_t src_h, uint32_t out_h)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
void am_sc1_set_input_format(uint32_t value)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
void am_sc1_set_output_format(uint32_t value)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
void am_sc1_set_buf_num(uint32_t num)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return;
...
int am_sc1_set_callback(acamera_context_ptr_t p_ctx, buffer_callback_t sc1_callback)
{
    ...
    g_sc->callback = sc1_callback;
    g_sc->ctx = p_ctx;
    return 0;
}

int am_sc1_system_init(void)
{
    int ret = 0;
...
int am_sc1_hw_init(int is_print, int clip_mode)
{
    uint32_t flag = 0;
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
...
int am_sc1_reset(void)
{
    return 0;
}

int am_sc1_stop(void)
{
    if (!g_sc) {
        pr_info("%d, g_sc is NULL.\n", __LINE__);
        return -1;
