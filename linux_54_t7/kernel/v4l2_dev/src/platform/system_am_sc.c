    ...
    if (g_sc->start_delay_cnt < START_DELAY_THR) {
        ...
        if (g_sc->start_delay_cnt == START_DELAY_THR) {
            ...
            for (flag = 0; flag < FRAME_DELAY_QUEUE - 1; flag ++) {
                g_sc->multi_camera.pre_frame[flag + 1] = g_sc->multi_camera.pre_frame[flag];
                g_sc->multi_camera.cam_id[flag + 1] = g_sc->multi_camera.cam_id[flag];
            }
            g_sc->multi_camera.pre_frame[0] = f_buf;
            g_sc->multi_camera.cam_id[0] = g_sc->cam_id_next;
            g_sc->last_end_frame = flag_ready;
            //reserve dummy buffer
            if (f_buf == am_ctx[g_sc->cam_id_next].temp_buf)
...
int am_sc_hw_init(int ctx_id, int is_print, int clip_mode)
{
    ...
    if ( am_ctx[ctx_id].info.c_height == 0 )
        am_ctx[ctx_id].info.c_height = am_ctx[ctx_id].info.src_w;
