...
/*notice: #define ENABLE_DI_YUV_DNR in file isp-v4l2-stream.h */
//#define ENABLE_DI_YUV_DNR

#ifndef ENABLE_DI_YUV_DNR
...
static int rgb2ycbcr[15] = {186,628,63,-103,-346,449,449,-408,-41,0,0,0,0,0,0}; //BT709
...
static void sc_do_workqueue( struct work_struct *work )
{
	...
	while (kfifo_out(&sc_workqueue.sc_fifo_out, &f_buff, sizeof(tframe_t*))) {
		metadata.width = g_sc->info.out_w;
		metadata.height = g_sc->info.out_h;
		metadata.frame_id = frame_id;
		metadata.frame_number = frame_id;
	...
	if (start_delay_cnt < start_delay_th) {
		...
		if (start_delay_cnt == start_delay_th) {
			...
			if (g_sc->crop_refresh_flag) {
				am_sc_hw_init(0, g_sc->crop_refresh_flag);
...
#else
				if (!kfifo_is_full(&sc_workqueue.sc_fifo_out)) {
					kfifo_in(&sc_workqueue.sc_fifo_out, &(pre_frame[frame_delay]), sizeof(tframe_t*));
				}
...
#else
	if (!kfifo_initialized(&sc_workqueue.sc_fifo_out)) {
		rtn = kfifo_alloc(&sc_workqueue.sc_fifo_out, PAGE_SIZE, GFP_KERNEL);
		if (rtn) {
			pr_info("alloc sc_workqueue fifo failed.\n");
			return rtn;
		}
	}
#endif
...
#else
	// kill workqueue
	kfifo_free(&sc_workqueue.sc_fifo_out);
#endif
...
uint32_t am_sc_get_startx(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return 0;
...
uint32_t am_sc_get_starty(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return 0;
...
void am_sc_set_fps(uint32_t c_fps, uint32_t t_fps)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc_set_startx(uint32_t startx)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc_set_starty(uint32_t starty)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
uint32_t am_sc_get_crop_width(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return 0;
...
uint32_t am_sc_get_crop_height(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return 0;
...
void am_sc_set_crop_width(uint32_t c_width)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc_set_crop_height(uint32_t c_height)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
#else
		kfifo_reset(&sc_workqueue.sc_fifo_out);
#endif
