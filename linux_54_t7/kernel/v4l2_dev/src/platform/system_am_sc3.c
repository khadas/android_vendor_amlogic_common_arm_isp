...
static void enable_isp_scale_new (
	...
	if ( stop_flag ) {
		...
		sc_reg_wr(ISP_SCWR_SYNC_DELAY, 0x4000100);
	...
	if (initial_en && stop_flag) {
	    ...
	    if ( mux_sel != 4 ) sc_wr_reg_bits(ISP_SCWR_TOP_CTRL, 5, 13, 3);
	}
...
static ssize_t sc3_frame_read(struct device *dev,
...
static ssize_t sc3_frame_write(struct device *dev,
...
static DEVICE_ATTR(sc3_frame, S_IRUGO | S_IWUSR, sc3_frame_read, sc3_frame_write);
	...
	if (start_delay_cnt < start_delay_th) {
		...
		if (start_delay_cnt == start_delay_th) {
			...
			if (g_sc->crop_refresh_flag) {
				am_sc3_hw_init(0, g_sc->crop_refresh_flag);
...
int am_sc3_parse_dt(struct device_node *node, int port)
{
	...
	device_create_file(&(t_sc->p_dev->dev), &dev_attr_sc3_frame);
	t_sc->port = port;
	g_sc = t_sc;
...
void am_sc3_deinit_parse_dt(void)
{
	...
	if (g_sc->p_dev != NULL)
		device_remove_file(&(g_sc->p_dev->dev), &dev_attr_sc3_frame);
...
void am_sc3_api_dma_buffer(tframe_t * data, unsigned int index)
{
	unsigned long flags;
...
uint32_t am_sc3_get_width(void)
{
	if (!g_sc) {
...
void am_sc3_set_width(uint32_t src_w, uint32_t out_w)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
uint32_t am_sc3_get_startx(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return 0;
...
uint32_t am_sc3_get_starty(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return 0;
...
void am_sc3_set_fps(uint32_t c_fps, uint32_t t_fps)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc3_set_startx(uint32_t startx)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc3_set_starty(uint32_t starty)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
uint32_t am_sc3_get_crop_width(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return 0;
...
uint32_t am_sc3_get_crop_height(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return 0;
...
void am_sc3_set_crop_width(uint32_t c_width)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc3_set_crop_height(uint32_t c_height)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc3_set_src_width(uint32_t src_w)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc3_set_src_height(uint32_t src_h)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
uint32_t am_sc3_get_height(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return -1;
...
uint32_t am_sc3_get_output_format(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return -1;
...
void am_sc3_set_height(uint32_t src_h, uint32_t out_h)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc3_set_input_format(uint32_t value)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc3_set_output_format(uint32_t value)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
void am_sc3_set_buf_num(uint32_t num)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return;
...
int am_sc3_set_callback(acamera_context_ptr_t p_ctx, buffer_callback_t sc3_callback)
{
	...
	g_sc->callback = sc3_callback;
	g_sc->ctx = p_ctx;
	return 0;
}

int am_sc3_system_init(void)
{
	int ret = 0;
...
int am_sc3_hw_init(int is_print, int clip_mode)
{
	uint32_t flag = 0;
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
	...
	if ( g_sc->info.c_width == 0 )
		g_sc->info.c_width = g_sc->info.out_w;

	if ( g_sc->info.c_height == 0 )
		g_sc->info.c_height = g_sc->info.out_h;
...
int am_sc3_reset(void)
{
	return 0;
}

int am_sc3_stop(void)
{
	if (!g_sc) {
		pr_info("%d, g_sc is NULL.\n", __LINE__);
		return -1;
