...
struct am_sc_context am3_ctx[CAM_CTX_NUM];
struct am_sc *g_sc3;
    ...
    unsigned int val)
{
    ...
    void __iomem *base = g_sc3->base_addr;
...
static void isp_mtx_setting(u8 ch_mode, s32 mode)
{
    ...
    if ((ch_mode == 0) && (mode == 1)
        && (g_sc3->isp_frame.reg_rgb_mode == 1))
        invert = true;
...
static void enable_isp_scale_new (
    ...
    if ( g_sc3->stop_flag || g_sc3->refresh ) isp_mif_setting(swap_uv, wr_mif);
    ...
    if ( g_sc3->stop_flag || g_sc3->refresh ) {
        if (dbg_mode) {
    ...
    if (initial_en && g_sc3->stop_flag) {
        sc_reg_rd(ISP_SCWR_TOP_DBG0, &val);
        if (val & (1 << 6))
            g_sc3->last_end_frame = 0;
        else
            g_sc3->last_end_frame = 1;
    ...
    if (g_sc3->refresh == false) pr_info(" finished isp scale setting \n");
}
...
static void init_sc_mif_setting(int ctx_id, ISP_MIF_t *mif_frame)
{
    ...
    if (g_sc3->stop_flag) {
        if (kfifo_len(&am3_ctx[ctx_id].sc_fifo_in) > 0) {
            retval = kfifo_out(&am3_ctx[ctx_id].sc_fifo_in, &buf, sizeof(tframe_t*));
            if (retval == sizeof(tframe_t*)) {
                g_sc3->multi_camera.cam_id[0] = ctx_id;
                g_sc3->multi_camera.pre_frame[0] = buf;
            } else {
    ...
    if (am3_ctx[ctx_id].info.out_fmt == NV12_GREY) {
        am3_ctx[ctx_id].ch_mode = 1;
    } else {
        am3_ctx[ctx_id].ch_mode = 0;
    }

    memset(mif_frame, 0, sizeof(ISP_MIF_t));
    plane_size = am3_ctx[ctx_id].info.out_w * am3_ctx[ctx_id].info.out_h;
    mif_frame->reg_little_endian = 1;
    if ((am3_ctx[ctx_id].info.in_fmt == RGB24) ||
        (am3_ctx[ctx_id].info.in_fmt == AYUV)) {
        if ((am3_ctx[ctx_id].info.out_fmt == RGB24) ||
            (am3_ctx[ctx_id].info.out_fmt == AYUV)) {
            mif_frame->reg_rgb_mode = 1;
        } else if (am3_ctx[ctx_id].info.out_fmt == NV12_YUV) {
            mif_frame->reg_rgb_mode = 2;
        } else if (am3_ctx[ctx_id].info.out_fmt == NV12_YVU) {
            mif_frame->reg_rgb_mode = 2;
        } else if (am3_ctx[ctx_id].info.out_fmt == UYVY) {
            mif_frame->reg_rgb_mode = 0;
        } else if (am3_ctx[ctx_id].info.out_fmt == NV12_GREY) {
            mif_frame->reg_rgb_mode = 2;
        } else if (am3_ctx[ctx_id].info.out_fmt == RAW16) {
            mif_frame->reg_rgb_mode = 0;
        } else if (am3_ctx[ctx_id].info.out_fmt == RAW_YUY2)
            mif_frame->reg_rgb_mode = 0;
    } else if ((am3_ctx[ctx_id].info.in_fmt == NV12_YUV) ||
            (am3_ctx[ctx_id].info.in_fmt == NV12_GREY)) {
        if (am3_ctx[ctx_id].info.out_fmt == NV12_GREY) {
            mif_frame->reg_rgb_mode = 2;
        }
    }
    ...
    mif_frame->reg_end_x = am3_ctx[ctx_id].info.out_w -1;
    mif_frame->reg_end_y = am3_ctx[ctx_id].info.out_h - 1;
    mif_frame->reg_hsizem1 = am3_ctx[ctx_id].info.out_w - 1;
    mif_frame->reg_vsizem1 = am3_ctx[ctx_id].info.out_h - 1;
    if (am3_ctx[ctx_id].ch_mode == 1) {
        mif_frame->reg_only_1ch = 1;
        mif_frame->reg_enable_3ch = 0;
        frame_size = plane_size;
        mif_frame->reg_canvas_strb_luma =
            am3_ctx[ctx_id].info.out_w;
        mif_frame->reg_canvas_strb_chroma = 0;
        mif_frame->reg_canvas_strb_r = 0;
        mif_frame->reg_hconv_mode = 2;
        mif_frame->reg_vconv_mode = 2;
        mif_frame->reg_rgb_mode = 2;
    } else if (am3_ctx[ctx_id].ch_mode == 3) {
        ...
        if (mif_frame->reg_rgb_mode == 0) {
            ...
            mif_frame->reg_canvas_strb_luma =
                am3_ctx[ctx_id].info.out_w * 2;
            mif_frame->reg_canvas_strb_chroma = 0;
            mif_frame->reg_canvas_strb_r = 0;
        } else if (mif_frame->reg_rgb_mode == 1) {
            frame_size = plane_size * 3;
            mif_frame->reg_canvas_strb_luma =
                am3_ctx[ctx_id].info.out_w * 3;
            mif_frame->reg_canvas_strb_chroma = 0;
            mif_frame->reg_canvas_strb_r = 0;
    ...
    pr_info("init_sc_mif_setting: %dx%d -> %dx%d, %x-%x-%x-%x-%x-%x, stride: %x-%x-%x\n",
        am3_ctx[ctx_id].info.src_w, am3_ctx[ctx_id].info.src_h,
        am3_ctx[ctx_id].info.out_w, am3_ctx[ctx_id].info.out_h,
        mif_frame->reg_canvas_baddr_luma,
        mif_frame->reg_canvas_baddr_chroma,
        mif_frame->reg_canvas_baddr_r,
        mif_frame->reg_canvas_baddr_luma_other,
...
static void sc_do_tasklet( unsigned long data )
{
    ...
    while (kfifo_out(&am3_ctx[ctx_id].sc_fifo_out, &f_buff, sizeof(tframe_t*))) {
        ...
        metadata.frame_number = am3_ctx[ctx_id].frame_id;
        metadata.line_size = (((3 * am3_ctx[ctx_id].info.out_w) + 127) & (~127));
        am3_ctx[ctx_id].callback(am3_ctx[ctx_id].ctx, f_buff, &metadata );
        am3_ctx[ctx_id].frame_id++;
    }
}
#endif
...
{
    if (ping) {
        g_sc3->isp_frame.reg_canvas_baddr_luma = f_buf->primary.address;
        g_sc3->isp_frame.reg_canvas_baddr_chroma = f_buf->secondary.address;
        g_sc3->isp_frame.reg_canvas_baddr_r = f_buf->secondary.address;
    } else {
        g_sc3->isp_frame.reg_canvas_baddr_luma_other = f_buf->primary.address;
        g_sc3->isp_frame.reg_canvas_baddr_chroma_other = f_buf->secondary.address;
        g_sc3->isp_frame.reg_canvas_baddr_r_other = f_buf->secondary.address;
    }
    isp_mif_addr(&g_sc3->isp_frame);
}

static void am_sc_swap_buf(int check_last, u32 flag_ready)
{
    u32 frame_delay = FRAME_DELAY_QUEUE - 1;
    g_sc3->last_end_frame = flag_ready;

    if (g_sc3->isp_frame.reg_pingpong_en) {
        if (g_sc3->no_ready_th || (check_last == 0)) {
            if (g_sc3->multi_camera.pre_frame[frame_delay - 1] && g_sc3->multi_camera.pre_frame[frame_delay - 2]) {
                tframe_t* temp_frame = g_sc3->multi_camera.pre_frame[frame_delay - 1];
                int cam_id = g_sc3->multi_camera.cam_id[frame_delay - 1];
                g_sc3->multi_camera.pre_frame[frame_delay - 1] = g_sc3->multi_camera.pre_frame[frame_delay - 2];
                g_sc3->multi_camera.pre_frame[frame_delay - 2] = temp_frame;
                g_sc3->multi_camera.cam_id[frame_delay - 1] = g_sc3->multi_camera.cam_id[frame_delay - 2];
                g_sc3->multi_camera.cam_id[frame_delay - 2] = cam_id;
            }
        }
    }
...
static irqreturn_t isp_sc_isr(int irq, void *data)
{
    if (g_sc3->start_delay_cnt < START_DELAY_THR) {
        ...
        if (g_sc3->start_delay_cnt == START_DELAY_THR) {
            ...
            if (g_sc3->isp_frame.reg_pingpong_en == 0)
                flag = 0;

            //pr_err("sc irq:%d, %d, %d, %d, f %d\n", g_sc3->cam_id_last, g_sc3->cam_id_current, g_sc3->cam_id_next, g_sc3->cam_id_next_next, (flag & (1 << 8)));
            spin_lock_irqsave( &g_sc3->sc_lock, flags );
            if (kfifo_len(&am3_ctx[g_sc3->cam_id_next].sc_fifo_in) > 0) {
                retval = kfifo_out(&am3_ctx[g_sc3->cam_id_next].sc_fifo_in, &f_buf, sizeof(tframe_t*));
                if (retval != sizeof(tframe_t*))
                ...
                else if (am3_ctx[g_sc3->cam_id_next_next].temp_buf)
                    f_buf = am3_ctx[g_sc3->cam_id_next_next].temp_buf;
                else if (am3_ctx[g_sc3->cam_id_current].temp_buf)
                    f_buf = am3_ctx[g_sc3->cam_id_current].temp_buf;
                else if (am3_ctx[g_sc3->cam_id_last].temp_buf)
                    f_buf = am3_ctx[g_sc3->cam_id_last].temp_buf;
            }
            spin_unlock_irqrestore( &g_sc3->sc_lock, flags );

            if (g_sc3->refresh && am3_ctx[g_sc3->cam_id_next].temp_buf) {
                am_sc3_hw_init(g_sc3->cam_id_next, 0, g_sc3->refresh);
                g_sc3->refresh = 0;
            }
            ...
            if (g_sc3->multi_camera.pre_frame[FRAME_DELAY_QUEUE-1]) {
#ifdef ENABLE_SC_BOTTOM_HALF_TASKLET
                if (!kfifo_is_full(&am3_ctx[g_sc3->multi_camera.cam_id[FRAME_DELAY_QUEUE-1]].sc_fifo_out)) {
            ...
            for (flag = FRAME_DELAY_QUEUE - 1; flag > 0; flag --) {
                g_sc3->multi_camera.pre_frame[flag] = g_sc3->multi_camera.pre_frame[flag - 1];
                g_sc3->multi_camera.cam_id[flag] = g_sc3->multi_camera.cam_id[flag - 1];
            }
            g_sc3->multi_camera.pre_frame[0] = f_buf;
            g_sc3->multi_camera.cam_id[0] = g_sc3->cam_id_next;
            g_sc3->last_end_frame = flag_ready;
            //reserve dummy buffer
            if (f_buf == am3_ctx[g_sc3->cam_id_last].temp_buf ||
                f_buf == am3_ctx[g_sc3->cam_id_next].temp_buf ||
                f_buf == am3_ctx[g_sc3->cam_id_current].temp_buf ||
                f_buf == am3_ctx[g_sc3->cam_id_next_next].temp_buf)
                g_sc3->multi_camera.pre_frame[0] = NULL;
        }
    }
    g_sc3->working = 0;
    return IRQ_HANDLED;
...
int am_sc3_parse_dt(struct device_node *node, int port)
{
    int rtn = -1;
    int irq = -1;
    struct resource rs;
    ...
    if (irq <= 0) {
        pr_err("%s:Error get isp-sc3 irq\n", __func__);
        goto irq_error;
    }
    ...
    spin_lock_init(&g_sc3->sc_lock);

    int i = 0;
    for (i = 0; i < CAM_CTX_NUM; i ++) {
        if (!kfifo_initialized(&am3_ctx[i].sc_fifo_in)) {
            rtn = kfifo_alloc(&am3_ctx[i].sc_fifo_in, PAGE_SIZE, GFP_KERNEL);
            if (rtn) {
        ...
        if (!kfifo_initialized(&am3_ctx[i].sc_fifo_out)) {
            rtn = kfifo_alloc(&am3_ctx[i].sc_fifo_out, PAGE_SIZE, GFP_KERNEL);
            if (rtn) {
...
void am_sc3_deinit_parse_dt(void)
{
    if (g_sc3 == NULL) {
        pr_err("Error g_sc3 is NULL\n");
        return;
    }

    int i = 0;
    for (i = 0; i < CAM_CTX_NUM; i ++) {
        kfifo_free(&am3_ctx[i].sc_fifo_in);
        kfifo_free(&am3_ctx[i].sc_fifo_out);
    }

    if (g_sc3->base_addr != NULL) {
        iounmap(g_sc3->base_addr);
        g_sc3->base_addr = NULL;
    }
...
void am_sc3_api_dma_buffer(int ctx_id, tframe_t * data, unsigned int index)
{
    unsigned long flags;

    if (am3_ctx[ctx_id].temp_buf == NULL) return;
    spin_lock_irqsave(&g_sc3->sc_lock, flags);
    tframe_t *buf = am3_ctx[ctx_id].temp_buf + index;
    ...
    if (!kfifo_is_full(&am3_ctx[ctx_id].sc_fifo_in)) {
        kfifo_in(&am3_ctx[ctx_id].sc_fifo_in, &buf, sizeof(tframe_t*));
    } else {
...
uint32_t am_sc3_get_width(int ctx_id)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return -1;
    }
    return am3_ctx[ctx_id].info.out_w;
}

void am_sc3_set_width(int ctx_id, uint32_t src_w, uint32_t out_w)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
    }
    if (am3_ctx[ctx_id].info.src_w == 0)
        am3_ctx[ctx_id].info.src_w = src_w;
    am3_ctx[ctx_id].info.out_w = out_w;
}

uint32_t am_sc3_get_startx(int ctx_id)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return 0;
    }
    return am3_ctx[ctx_id].info.startx;
}

uint32_t am_sc3_get_starty(int ctx_id)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return 0;
    }
    return am3_ctx[ctx_id].info.starty;
}

void am_sc3_set_fps(int ctx_id, uint32_t c_fps, uint32_t t_fps)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
...
uint32_t am_sc3_get_fps(int ctx_id)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return 0;
    }
    return am3_ctx[ctx_id].info.t_fps;
}

void am_sc3_set_startx(int ctx_id, uint32_t startx)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
    }
    am3_ctx[ctx_id].info.startx = startx;
}

void am_sc3_set_starty(int ctx_id, uint32_t starty)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
    }
    am3_ctx[ctx_id].info.starty = starty;
}

uint32_t am_sc3_get_crop_width(int ctx_id)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return 0;
    }
    return am3_ctx[ctx_id].info.c_width;
}

uint32_t am_sc3_get_crop_height(int ctx_id)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return 0;
    }
    return am3_ctx[ctx_id].info.c_height;
}

void am_sc3_set_crop_width(int ctx_id, uint32_t c_width)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
    }
    am3_ctx[ctx_id].info.c_width = c_width;
}

void am_sc3_set_crop_height(int ctx_id, uint32_t c_height)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
    }
    am3_ctx[ctx_id].info.c_height = c_height;
}

void am_sc3_set_crop_enable(int ctx_id)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
...
void am_sc3_set_src_width(int ctx_id, uint32_t src_w)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
...
void am_sc3_set_src_height(int ctx_id, uint32_t src_h)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
    }
    am3_ctx[ctx_id].info.src_h = src_h;
}

uint32_t am_sc3_get_height(int ctx_id)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return -1;
    }
    return am3_ctx[ctx_id].info.out_h;
}

uint32_t am_sc3_get_output_format(int ctx_id)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return -1;
    }
    return am3_ctx[ctx_id].info.out_fmt;
}


void am_sc3_set_height(int ctx_id, uint32_t src_h, uint32_t out_h)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
    }
    if (am3_ctx[ctx_id].info.src_h == 0)
        am3_ctx[ctx_id].info.src_h = src_h;
    am3_ctx[ctx_id].info.out_h = out_h;
}

void am_sc3_set_input_format(int ctx_id, uint32_t value)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
    }
    am3_ctx[ctx_id].info.in_fmt = RGB24;
}

void am_sc3_set_output_format(int ctx_id, uint32_t value)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
    }
    am3_ctx[ctx_id].info.out_fmt = value;
}

void am_sc3_set_buf_num(int ctx_id, uint32_t num)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return;
    }
    am3_ctx[ctx_id].req_buf_num = num;
}

int am_sc3_set_callback(acamera_context_ptr_t p_ctx, buffer_callback_t sc3_callback)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return -1;
    }
    am3_ctx[p_ctx->fsm_mgr.ctx_id].callback = sc3_callback;
    am3_ctx[p_ctx->fsm_mgr.ctx_id].ctx = p_ctx;
    return 0;
}

int am_sc3_system_init(int ctx_id)
{
    int ret = 0, i = 0;
    ...
    if (am3_ctx[ctx_id].req_buf_num == 0)
        return 0;

    if (!am3_ctx[ctx_id].temp_buf) {
        am3_ctx[ctx_id].temp_buf = (tframe_t*)kmalloc( sizeof(tframe_t) * (am3_ctx[ctx_id].req_buf_num), GFP_KERNEL | __GFP_NOFAIL);
    }

    if (g_sc3->stop_flag == false) return 0;

    for (i = 0; i < FRAME_DELAY_QUEUE; i ++) {
        g_sc3->multi_camera.pre_frame[i] = NULL;
        g_sc3->multi_camera.cam_id[i] = 0;
    }
...
int am_sc3_hw_init(int ctx_id, int is_print, int clip_mode)
{
    uint32_t flag = 0;
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
    ...
    if ((g_sc3->stop_flag == false) && !clip_mode) {
    ...
    unsigned long flags;
    spin_lock_irqsave( &g_sc3->sc_lock, flags );
    if ( g_sc3->stop_flag || g_sc3->refresh ) init_sc_mif_setting(ctx_id, &g_sc3->isp_frame);
    if (am3_ctx[ctx_id].info.in_fmt == RGB24) {
        if (am3_ctx[ctx_id].info.out_fmt == RGB24) {
            mtx_mode = 0;
        } else if ((am3_ctx[ctx_id].info.out_fmt == NV12_YUV) ||
                (am3_ctx[ctx_id].info.out_fmt == NV12_YVU) ||
                (am3_ctx[ctx_id].info.out_fmt == UYVY) ||
                (am3_ctx[ctx_id].info.out_fmt == AYUV)) {
            mtx_mode = 1;
        } else if (am3_ctx[ctx_id].info.out_fmt == NV12_GREY) {
            mtx_mode = 1;
    ...
    } else if (am3_ctx[ctx_id].info.in_fmt == AYUV) {
        if ((am3_ctx[ctx_id].info.out_fmt == NV12_YUV) ||
            (am3_ctx[ctx_id].info.out_fmt == NV12_YVU) ||
            (am3_ctx[ctx_id].info.out_fmt == UYVY) ||
            (am3_ctx[ctx_id].info.out_fmt == AYUV)) {
            mtx_mode = 0;
        } else if (am3_ctx[ctx_id].info.out_fmt == RGB24) {
            mtx_mode = 2;
        } else if (am3_ctx[ctx_id].info.out_fmt == NV12_GREY) {
            mtx_mode = 0;
        }
    ...
    if ( am3_ctx[ctx_id].info.c_width == 0 )
        am3_ctx[ctx_id].info.c_width = am3_ctx[ctx_id].info.out_w;

    if ( am3_ctx[ctx_id].info.c_height == 0 )
        am3_ctx[ctx_id].info.c_height = am3_ctx[ctx_id].info.out_h;

    if ( ( am3_ctx[ctx_id].info.c_width != am3_ctx[ctx_id].info.src_w ) || ( am3_ctx[ctx_id].info.c_height != am3_ctx[ctx_id].info.src_h ) )
        am3_ctx[ctx_id].info.clip_sc_mode |= CLIP_MODE;
    else
        am3_ctx[ctx_id].info.clip_sc_mode &= ~CLIP_MODE;
    ...
    if ( g_sc3->port < 3 ) {
        if ( ( am3_ctx[ctx_id].info.c_width != am3_ctx[ctx_id].info.out_w ) || ( am3_ctx[ctx_id].info.c_height != am3_ctx[ctx_id].info.out_h ) )
            am3_ctx[ctx_id].info.clip_sc_mode |= SC_MODE;
    }

    if ( is_print ) pr_info("ctx_id-%d, src_w = %d, src_h = %d, out_w = %d, out_h = %d, crop_w = %d, crop_h = %d, clip = %d, in_fmt:%d, out_fmt:%d\n",
            ctx_id, am3_ctx[ctx_id].info.src_w, am3_ctx[ctx_id].info.src_h, am3_ctx[ctx_id].info.out_w, am3_ctx[ctx_id].info.out_h, am3_ctx[ctx_id].info.c_width,
            am3_ctx[ctx_id].info.c_height, am3_ctx[ctx_id].info.clip_sc_mode, am3_ctx[ctx_id].info.in_fmt, am3_ctx[ctx_id].info.out_fmt);
    ...
    enable_isp_scale_new(1, 0, dbg_mode, am3_ctx[ctx_id].info.clip_sc_mode & CLIP_MODE, am3_ctx[ctx_id].info.startx,
        am3_ctx[ctx_id].info.startx + am3_ctx[ctx_id].info.c_width - 1, am3_ctx[ctx_id].info.starty,  am3_ctx[ctx_id].info.starty + am3_ctx[ctx_id].info.c_height - 1,
        am3_ctx[ctx_id].info.src_w, am3_ctx[ctx_id].info.src_h, am3_ctx[ctx_id].info.out_w, am3_ctx[ctx_id].info.out_h, mux_sel, am3_ctx[ctx_id].info.clip_sc_mode & SC_MODE, mtx_mode,
        swap_uv, am3_ctx[ctx_id].ch_mode, &g_sc3->isp_frame);
...
int am_sc3_start(int ctx_id)
{
    ...
    if (g_sc3->stop_flag == false) return 0;
#ifdef ENABLE_SC_BOTTOM_HALF_TASKLET
    tasklet_init( &g_sc3->sc_tasklet.tasklet_obj, sc_do_tasklet, (unsigned long)&g_sc3->sc_tasklet );
#endif
    ret = request_irq(g_sc3->irq, isp_sc_isr, IRQF_SHARED | IRQF_TRIGGER_RISING,
        "isp-sc3-irq", (void *)g_sc3);
    pr_info("%s irq = %d, ret = %d\n", __func__,g_sc3->irq, ret);

    /* switch int to sync reset for start and delay frame */
    sc_wr_reg_bits(ISP_SCWR_TOP_CTRL, 1, 3, 1);
    g_sc3->stop_flag = false;

    return 0;
}

int am_sc3_reset(int ctx_id)
{
    return 0;
}

int am_sc3_stop(int ctx_id)
{
    int i = 0;
    ...
    if (!g_sc3->stop_flag && !g_sc3->user) {
        ...
        free_irq(g_sc3->irq, (void *)g_sc3);
#ifdef ENABLE_SC_BOTTOM_HALF_TASKLET
        tasklet_kill( &g_sc3->sc_tasklet.tasklet_obj );
#endif
    }

    spin_lock_irqsave( &g_sc3->sc_lock, flags );
    if (am3_ctx[ctx_id].temp_buf != NULL) {
        kfree(am3_ctx[ctx_id].temp_buf);
        am3_ctx[ctx_id].temp_buf = NULL;
    }

    kfifo_reset(&am3_ctx[ctx_id].sc_fifo_out);
    kfifo_reset(&am3_ctx[ctx_id].sc_fifo_in);
    am3_ctx[ctx_id].frame_id = 0;

    am3_ctx[ctx_id].info.startx = 0;
    am3_ctx[ctx_id].info.starty = 0;
    am3_ctx[ctx_id].info.c_width = 0;
    am3_ctx[ctx_id].info.c_height = 0;
    am3_ctx[ctx_id].info.src_w = 0;
    am3_ctx[ctx_id].info.src_h = 0;
    am3_ctx[ctx_id].info.clip_sc_mode = 0;
    spin_unlock_irqrestore( &g_sc3->sc_lock, flags );

    return 0;
}

int am_sc3_system_deinit(int ctx_id)
{
    if (!g_sc3) {
        pr_info("%d, g_sc3 is NULL.\n", __LINE__);
        return -1;
...
int am_sc3_hw_deinit(int ctx_id)
{
    return 0;
}

void am_sc3_hw_enable(void)
{
    sc_wr_reg_bits(ISP_SCWR_TOP_CTRL, 1, 1, 1);
    sc_wr_reg_bits(ISP_SCWR_TOP_CTRL, 1, 0, 1);
}

void am_sc3_hw_disable(void)
{
    sc_wr_reg_bits(ISP_SCWR_TOP_CTRL, 0, 1, 1);
    sc_wr_reg_bits(ISP_SCWR_TOP_CTRL, 0, 0, 1);
}

void am_sc3_set_workstatus(uint32_t status)
{
    int i = 0, ret = 0;
    uint32_t c_width = am3_ctx[0].info.c_width;
    uint32_t c_height = am3_ctx[0].info.c_height;
    uint32_t src_w = am3_ctx[0].info.src_w;
    uint32_t src_h = am3_ctx[0].info.src_h;
    uint32_t out_w = am3_ctx[0].info.out_w;
    uint32_t out_h = am3_ctx[0].info.out_h;
    uint32_t out_fmt = am3_ctx[0].info.out_fmt;

    if (g_sc3->stop_flag) return;

    g_sc3->cam_id_last = system_get_last_api_context();
    ...
    g_sc3->working = status;
    if (g_sc3->user > 1) {
        for (i = 0; i < CAM_CTX_NUM - 1; i ++) {
            if (c_width != am3_ctx[i + 1].info.c_width) {
                ret = -1;
                break;
            } else if (c_height != am3_ctx[i + 1].info.c_height) {
                ret = -1;
                break;
            } else if (src_w != am3_ctx[i + 1].info.src_w) {
                ret = -1;
                break;
            } else if (src_h != am3_ctx[i + 1].info.src_h) {
                ret = -1;
                break;
            } else if (out_w != am3_ctx[i + 1].info.out_w) {
                ret = -1;
                break;
            } else if (out_h != am3_ctx[i + 1].info.out_h) {
                ret = -1;
                break;
            } else if (out_fmt != am3_ctx[i + 1].info.out_fmt) {
                ret = -1;
                break;
            }
        }
    } else {
        if ((g_sc3->cam_id_current != g_sc3->cam_id_next) || (g_sc3->cam_id_current != g_sc3->cam_id_next_next))
            g_sc3->refresh = 1;
    }
...
uint32_t am_sc3_get_workstatus(void)
{
    return g_sc3->working;
}

void am_sc3_set_camid(uint32_t status)
{
    g_sc3->cam_id_last = system_get_last_api_context();
    g_sc3->cam_id_current = system_get_current_api_context();
    g_sc3->cam_id_next = system_get_next_api_context();
