    ...
    if (g_sc3->start_delay_cnt < START_DELAY_THR) {
        ...
        if (g_sc3->start_delay_cnt == START_DELAY_THR) {
            ...
            for (flag = 0; flag < FRAME_DELAY_QUEUE - 1; flag ++) {
                g_sc3->multi_camera.pre_frame[flag + 1] = g_sc3->multi_camera.pre_frame[flag];
                g_sc3->multi_camera.cam_id[flag + 1] = g_sc3->multi_camera.cam_id[flag];
            }
            g_sc3->multi_camera.pre_frame[0] = f_buf;
            g_sc3->multi_camera.cam_id[0] = g_sc3->cam_id_next;
            g_sc3->last_end_frame = flag_ready;
            //reserve dummy buffer
            if (f_buf == am3_ctx[g_sc3->cam_id_next].temp_buf)
